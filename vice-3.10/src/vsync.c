...
void vsync_do_end_of_line(void)
{
    ...
    if (tick_delta >= tick_between_sync) {
        ...
        if (warp_enabled) {
            ...
            if ((int64_t)ticks_until_target > (int64_t)tick_per_second()) {
                /* We are more than a second ahead. Sleep it off. */
                if (tick_based_sync_timing) {
                    mainlock_yield_and_sleep(ticks_until_target);
                }
            } else if ((int64_t)ticks_until_target < -(int64_t)tick_per_second()) {
                ...
                /* We are within reasonable sync range */
                if (tick_based_sync_timing && (int64_t)ticks_until_target > 0) {
                    mainlock_yield_and_sleep(ticks_until_target);
                } else {
                    /* Behind, but catching up, or timing not tick based. Yield to UI. */
                    mainlock_yield();
                }
            }
        }
