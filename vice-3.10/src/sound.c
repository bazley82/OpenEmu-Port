...
bool sound_flush(void)
{
    ...
    */
    ...
    const sound_device_t *pdev = snddata.playdev;
    while (!warp_mode_enabled && pdev) {

        /* Check if device was closed by another thread */
        if (snddata.playdev != pdev) {
             break;
        }

        if (pdev->bufferspace) {
            space = pdev->bufferspace();
        } else {
        ...
        if (space) {
            ...
            /* Use cached pointer 'pdev' which is statically allocated and safe even if closed */
            if (pdev->write(snddata.buffer, nr * snddata.sound_output_channels)) {
                sound_error("write to sound device failed.");
                goto done;
            }
