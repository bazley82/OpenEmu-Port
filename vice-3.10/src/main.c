...
{
    printf("[VICE] main_program called with argc=%d\n", argc);
    int i, n;
    int reserr;
    char *cmdline;
    ...
    */
    ...
    printf("[VICE DEBUG] machine_context addr: %p\n", (void *)&machine_context);
    printf("[VICE DEBUG] machine_context.cia1: %p\n", machine_context.cia1);
    if (!machine_context.cia1) {
        printf("[VICE DEBUG] FATAL: machine_context.cia1 is NULL!\n");
    }
    drive_setup_context();
    machine_early_init();
    ...
    printf("[VICE] main_program starting\n");
    if (init_main() < 0) {
        printf("[VICE] main_program: init_main failed\n");
        return -1;
    }
    printf("[VICE] main_program: init_main success\n");
...
void main_loop_forever(void)
{
    printf("[VICE] main_loop_forever starting\n");
